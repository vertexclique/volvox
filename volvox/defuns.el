...
(defun volvox-zap-to-char (arg char)
...
(defun volvox-ido-find-config ()
  (interactive)
  (find-file
   (concat "~/.emacs.d/volvox/"
           (ido-completing-read "Config file: "
                                (directory-files "~/.emacs.d/volvox/"
                                                 nil
                                                 "^[^.]")))))

(defun volvox-delete-till-end-of-buffer ()
...
(defun volvox-ido-find-project ()
  (interactive)
  (find-file
   (concat "~/Code/" (ido-completing-read "Project: "
                           (directory-files "~/Code/" nil "^[^.]")))))

(defun volvox-goto-config ()
  (interactive)
  (find-file "~/.emacs.d/volvox.el"))

;; fix kill-word
(defun volvox-kill-word (arg)
  ...
  (let ((whitespace-regexp "\\s-+"))
    (kill-region (point)
                 (cond
                  ...
                  ((looking-at "\n") (kill-line) (volvox-kill-word arg))
                  (t (forward-word arg) (point))))))

(defun volvox-backward-kill-word (arg)
...
(defun volvox-shebang-to-mode ()
  ...
  (let*
    ...
    (when (functionp mode-fn)
      (funcall mode-fn))))
;(add-hook 'find-file-hook 'volvox-shebang-to-mode)

; duplicate the current line
(defun volvox-duplicate-line ()
  (interactive)
    (beginning-of-line)
...
(defun vendor (library)
  (let* ((file (symbol-name library))
         ...
         (volvox (concat "~/.emacs.d/volvox/" file)))
    (cond
     ((file-directory-p normal) (add-to-list 'load-path normal) (require library))
     ((file-directory-p suffix) (add-to-list 'load-path suffix) (require library))
     ((file-exists-p suffix) (require library)))
    (when (file-exists-p (concat volvox ".el"))
      (load volvox))))

(defun volvox-backward-kill-line ()
  (interactive)
  (kill-line 0))

(require 'thingatpt)
(defun volvox-change-num-at-point (fn)
...
(defun volvox-inc-num-at-point ()
  (interactive)
  (volvox-change-num-at-point '+))

(defun volvox-dec-num-at-point ()
  (interactive)
  (volvox-change-num-at-point '-))
...
(defun gist-buffer-confirm (&optional private)
  ...
  (defun volvox/c-electric-brace (arg)
